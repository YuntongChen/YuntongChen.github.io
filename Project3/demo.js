...
// FILTER
const negativeUsers = map.filter((user) => {
    return user.negative
})

const positiveUsers = map.filter((user) => {
    return user.positive
})

const neutralUsers = map.filter((user) => {
    return user.neutral
})

// SORT 
const usersSortedByType = map.sort((negative, positive) => {
